...
function extractParamsFromQueryKey(queryKey: ShortPostQueryKey): {created_by_user?:User['id']} {
  const byUserIndex = queryKey.findIndex(v => v === 'byUser')
  const fromFollowedUsersIndex  = queryKey.findIndex(v=> v=== 'fromFollowedUsers')
...
export const fetchShortPosts = async (queryKeys:ShortPostQueryKey):Promise<(ShortPost)[]>=>{

  const queryParams = extractParamsFromQueryKey(queryKeys)
  const resJson = await makeRequest<Array<ShortPost>>('GET', 'shortPost',{isAuthenticated:true, queryParams})
  return resJson
...
export const isOptimisticShortPost = (post:OptimisticShortPost|ShortPost):post is OptimisticShortPost => {
return 'track' in post
}